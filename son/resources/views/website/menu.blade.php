				...
				<div class="xt-ct-menu">
                    ...
                    <div class="xtlab-ctmenu-sub">
                    	<ul class="sub-menu sub-menu-background">
                    		<?php $danhmucsanphamMenu1 = DB::table('danhmucsanpham')->where('danhmuccha_id', 0)->where('status', 1)->orderBy('vitri', 'ASC')->limit(7)->get() ?>
                    		@foreach($danhmucsanphamMenu1 as $item1)
                    		<li>
                    			<a href="{{ route('indexCode', ['code' => $item1->code]) }}"><img src="images/icon-dot.png" alt="Icon Menu"> <span>{{ $item1->name}}</span></a>
                    		...
                    		<?php $danhmucsanphamMenu11 = DB::table('danhmucsanpham')->where('danhmuccha_id', 0)->where('status', 1)->orderBy('vitri', 'ASC')->skip(7)->take(8)->get() ?>
                    		@if(count($danhmucsanphamMenu11) > 0)
                    		<div class="xtlab-ctmenu-sub-hidden">
				                <ul class="ul-last">
				                	@foreach($danhmucsanphamMenu11 as $item11)
                                    <li>
		                    			<a href="{{ route('indexCode', ['code' => $item11->code]) }}"><img src="images/icon-dot.png" alt="Icon Menu"> <span>{{ $item11->name}}</span></a>
		                    			<?php $danhmucsanphamMenu21 = DB::table('danhmucsanpham')->where('danhmuccha_id', $item11->id)->where('status', 1)->orderBy('vitri', 'ASC')->get() ?>
